...
// IMPORTANT: Load .env.local BEFORE any other imports that use process.env
import { config } from 'dotenv'
config({ path: '.env.local', override: true })

// Now safe to import modules that read DATABASE_URL
...
async function main() {
  ...
  try {
    ...
    console.log(`  bot_reply_enabled: ${settings.bot_reply_enabled}`)
  } catch (err) {
  ...
  try {
    const logs = await db.moderationActionLog.findMany({
  ...
  if (isDryRunEnv || isDryRunDb) {
    ...
    console.log('   FIX: Run `npx tsx scripts/disable-dry-run.ts` and unset the env var')
  } else {
