...
interface PayoutSummary {
    ...
    total_weighted_sweet_coins: number
    dollar_per_sweet_coin: number
    dollar_per_weighted_sweet_coin: number
    total_payout: number
    budget: number
...
export default function PayoutsPage() {
    ...
    const exportToCSV = () => {
        ...
        const headers = payoutData.summary.rank_bonus
            ? ['Rank', 'Username', 'Telegram', 'SweetCoins', 'Bonus', 'Weighted SweetCoins', 'Payout ($)', 'Percentage (%)']
            : ['Rank', 'Username', 'Telegram', 'SweetCoins', 'Payout ($)', 'Percentage (%)']
        ...
        const rows = payoutData.payouts.map(p => {
            ...
            if (payoutData.summary.rank_bonus) {
                return [
                    ...
                    p.username,
                    telegramDisplay,
                    (p.sweet_coins ?? 0).toString(),
                    (p.multiplier ?? 1) > 1 ? `+${(((p.multiplier ?? 1) - 1) * 100).toFixed(0)}%` : '-',
                    (p.weighted_sweet_coins ?? 0).toFixed(2),
        ...
        rows.push([])
        rows.push(['Summary'])
        rows.push(['Total SweetCoins', (payoutData.summary.total_sweet_coins ?? 0).toString()])
        if (payoutData.summary.rank_bonus) {
            rows.push(['Total Weighted SweetCoins', (payoutData.summary.total_weighted_sweet_coins ?? 0).toString()])
            rows.push(['Rank Bonus', 'Enabled (1st +100%, 2nd +75%, 3rd +50%; rest even)'])
        }
        rows.push(['Dollar per SweetCoin', `$${(payoutData.summary.dollar_per_sweet_coin ?? 0).toFixed(6)}`])
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                            <div className="bg-white dark:bg-kick-surface rounded-lg shadow-sm border border-gray-200 dark:border-kick-border p-4">
                                <div className="text-sm text-gray-600 dark:text-kick-text-secondary">Total SweetCoins</div>
                                <div className="text-2xl font-bold text-gray-900 dark:text-kick-text">
                                    {(payoutData.summary.total_sweet_coins ?? 0).toLocaleString()}
                                </div>
                            </div>
                            <div className="bg-white dark:bg-kick-surface rounded-lg shadow-sm border border-gray-200 dark:border-kick-border p-4">
                                <div className="text-sm text-gray-600 dark:text-kick-text-secondary">$/SweetCoin</div>
                                <div className="text-2xl font-bold text-kick-green">
                                    ${(payoutData.summary.dollar_per_sweet_coin ?? 0).toFixed(4)}
                                </div>
                            </div>
                            <div className="bg-white dark:bg-kick-surface rounded-lg shadow-sm border border-gray-200 dark:border-kick-border p-4">
                        ...
                        {payoutData.payouts.length === 0 ? (
                            <div className="bg-white dark:bg-kick-surface rounded-lg shadow-sm border border-gray-200 dark:border-kick-border p-12 text-center">
                                <p className="text-gray-600 dark:text-kick-text-secondary">
                                    No SweetCoins were earned during this stream session.
                                </p>
                            </div>
                        ) : (
                                    ...
                                    <table className="w-full">
                                        <thead>
                                            <tr className="border-b border-gray-200 dark:border-kick-border bg-gray-50 dark:bg-kick-dark">
                                                ...
                                                <th className="text-right py-3 px-4 text-sm font-semibold text-gray-700 dark:text-kick-text-secondary">SweetCoins</th>
                                                ...
                                                >
                                                    ...
                                                    <td className="py-4 px-4 text-right">
                                                        <span className="font-semibold text-kick-purple">
                                                            {(entry.sweet_coins ?? 0).toLocaleString()}
                                                        </span>
                                                    </td>
                                                    {payoutData.summary.rank_bonus && (
                                                        <td className="py-4 px-4 text-right">
                        ...
                        {payoutData.summary.top_n && payoutData.summary.total_participants > payoutData.summary.participant_count && (
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                                <p className="text-blue-800 dark:text-blue-200 text-sm">
                                    <strong>Rank Filter (up to rank {payoutData.summary.top_n}):</strong> Showing {payoutData.summary.participant_count} recipients out of {payoutData.summary.total_participants} total participants. All users with the same SweetCoins share the same rank. The ${(payoutData.summary.budget ?? 0).toFixed(2)} budget is split only among these ranks.
                                </p>
                            </div>
                        )}
