...
async function kickApiRequest<T>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            // Handle 401/403 - token might be expired, try to refresh if it's a user token
            if ((response.status === 401 || response.status === 403) && attempt === 0) {
                console.warn(`Got ${response.status}, attempting token refresh`)
                clearTokenCache()
        ...
        for (let i = 0; i < userIds.length; i += batchSize) {
            ...
            while (retryCount <= maxRetries && !batchSuccess) {
                ...
                try {
                    ...
                    } else if ((response.status === 401 || response.status === 403) && retryCount < maxRetries) {
                        // Token expired - try to refresh
                        console.warn(`[Kick API] Got ${response.status} for users batch, attempting token refresh (attempt ${retryCount + 1})`)
                        clearTokenCache()
...
/**
 * Refresh moderator token using refresh token
 */
async function refreshModeratorToken(moderatorUserId: bigint, refreshTokenEncrypted: string): Promise<string | null> {
    ...
    try {
        ...
        // Debug: Check what credentials we are actually using
        console.log(`[Kick API] Refreshing with Client ID: ${clientId.substring(0, 5)}...${clientId.slice(-3)}`)
        console.log(`[Kick API] Refreshing with Client Secret: ${clientSecret.substring(0, 3)}...${clientSecret.slice(-3)}`)
        ...
        if (response.ok) {
            ...
            await db.user.update({
                where: { kick_user_id: moderatorUserId },
                data: {
                    access_token_hash: hashToken(data.access_token),
                    refresh_token_hash: data.refresh_token ? hashToken(data.refresh_token) : undefined,
                    access_token_encrypted: encryptToken(data.access_token),
    ...
    try {
        ...
        try {
            ...
            if (moderator.refresh_token_encrypted) {
                return await refreshModeratorToken(moderator.kick_user_id, moderator.refresh_token_encrypted)
            }
...
export async function moderationBan(params: {
    ...
    try {
        ...
        // Get moderator info for potential token refresh
        const moderator = await db.user.findFirst({
            where: {
                username: {
                    equals: MODERATOR_USERNAME,
        ...
        const body: any = {
            broadcaster_user_id: typeof params.broadcaster_user_id === 'bigint'
                ? Number(params.broadcaster_user_id)
                : typeof params.broadcaster_user_id === 'string'
                    ? parseInt(params.broadcaster_user_id, 10)
        ...
        // Try the request, refresh token on 401 and retry once
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            // Kick swagger uses /moderation/bans (plural)
            const response = await fetch(`${KICK_API_BASE}/moderation/bans`, {
                method: 'POST',
                headers,
            ...
            // If 401/403 and we have a refresh token, try refreshing and retry once
            if ((response.status === 401 || response.status === 403) && attempt === 0 && moderator.refresh_token_encrypted) {
                const errorBody = errorText ? errorText.substring(0, 200) : 'No error body'
                console.log(`[Kick API] Token issue (${response.status}), attempting to refresh moderator token... Error: ${errorBody}`)
                const refreshedToken = await refreshModeratorToken(moderator.kick_user_id, moderator.refresh_token_encrypted)
                if (refreshedToken) {
                    moderatorToken = refreshedToken
                    // Refresh moderator info for next attempt
                    const updatedModerator = await db.user.findFirst({
                        where: { kick_user_id: moderator.kick_user_id },
                        select: { refresh_token_encrypted: true },
                    })
                    if (updatedModerator) {
                        moderator.refresh_token_encrypted = updatedModerator.refresh_token_encrypted || moderator.refresh_token_encrypted
                    }
        ...
        return { success: false, error: 'Failed after token refresh attempt' }
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error'
 ...
 * @param type - Message type: 'user' or 'bot' (default: 'user')
 */
export async function sendModeratorChatMessage(params: {
    ...
    try {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            if (response.ok) {
                // Kick returns JSON; log a small, safe preview so we can confirm the message id/timestamp.
                const responseText = await response.text().catch(() => '')
                let preview = responseText
                try {
                    const json = responseText ? JSON.parse(responseText) : null
                    preview = json ? JSON.stringify(json).substring(0, 300) : responseText.substring(0, 300)
                } catch {
                    preview = responseText.substring(0, 300)
                }
                console.log(`[Kick API] Successfully sent chat message: ${params.content.substring(0, 50)}... | response=${preview}`)
                return { success: true }
            }
